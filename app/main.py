    ...
    if row.provider != "ollama" and not settings.allow_external_in_internal_modes:
        raise HTTPException(
            ...
            detail=(
                ...
                "Set ALLOW_EXTERNAL_IN_INTERNAL_MODES=true (or unset it; the default is true)."
            ),
        )
    return row
...
def gateway_landing_page():
    return RedirectResponse(url="/gateway/login", status_code=307)
    ...
    try:
        ...
        if mode == "chat":
            ...
            with SessionLocal() as db:
                ...
                model_key = model_row.model_key
                provider = model_row.provider
            answer = run_chat_mode(client=client, model=target_model, prompt=prompt)
            _save_log(prompt=prompt, answer=answer)
            return ModeRunResponse(
                mode=mode,
                result={"text": answer, "model_id": model_key, "provider": provider},
            )
